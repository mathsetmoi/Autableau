    ...
    function rendrePublier(p) {
        if (!window.MonDossier || !MonDossier.pret()) {
            p.appendChild(h('div', { class: 'pw-bloc' },
                h('div', { class: 'pw-bloc-titre' }, 'Votre Drive n\'est pas ouvert'),
                h('p', { class: 'pub-texte' }, 'Les séances publiées vont dans un dossier de votre Drive. Ouvrez-le d\'abord : '
                    + 'tiroir de droite, onglet « Mes tableaux », « Mon Drive ».')));
            return;
        }
        if (!regle()) {
            p.appendChild(h('div', { class: 'pw-bloc' },
        ...
        const titreDefaut = MonDossier.nomCourant();
